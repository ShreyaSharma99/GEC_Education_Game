...
'''
Probable To Dos -
1. Not needed right now ------- Bert vocabulory for prediction vocabulory set
2. Use BART encoder - decoder model (pre-trained)
3. Use start and end index information as 
    a. Appended as Positional encoder 
    b. As a new token in the embedding


Mattia's Sugegstions - 
1. Batch size - as large as poosible
2. Correct the accuracy calculation for all
4. Add start, end index information
3. Improve the decoder - 
    a. Transoformer decoder layer
    b. T5 encoder + decoder and then Transoformer decoder layer
'''
...
for index, row in df.iterrows():
    ...
    if text in sent_actions:
        sent_actions[text].append(text)
    else:
        sent_actions[text] = [text]
...
# vocab_10gec.append("<START>")
# vocab_10gec.append("<END>")
    ...
    def __init__(self, df, tokenizer):
        ...
        for label in df["action"]:
            ...
            if ind_list[0] == ind_list[1]: #Add
                ...
                act = 1
            words = " , ".join([t.strip() for t in ind_list[2:]])
            word_index = []
...
def train(model, tokenizer, train_data, val_data, learning_rate, epochs, result_folder):
    ...
    path = os.path.join("model_files_bart_transformer")
    ...
    for epoch_num in range(epochs):
        ...
        total_acc_train = 0
        part_acc_train = np.asarray([0,0,0,0])
        total_loss_train = 0
        ...
        for train_input, train_label in tqdm(train_dataloader):
            ...
            # print("input_id - " ,input_id[0, :].size())
            # word_sent = tokenizer.decode(input_id[0, :])
            # print(word_sent[word_sent.index("<s>")+3:word_sent.index("</s>")]) 
            ...
            acc, partial_acc = batch_prediction_accuracy(action, start_index, end_index, output_seq, tokenizer, input_id, sent_actions, vocab_10gec)
            # action_accuracy(action, start_index, end_index, output_seq, train_label)
        ...
        total_acc_val = 0
        part_acc_val = np.asarray([0,0,0,0])
        total_loss_val = 0
        ...
        with torch.no_grad():
            ...
            for val_input, val_label in val_dataloader:
                ...
                total_acc_val += acc
                part_acc_val += np.asarray(partial_acc)
...
def evaluate(model, tokenizer, test_data):
    ...
    total_acc_test = 0
    part_acc_test = np.asarray([0,0,0,0])
    ...
    with torch.no_grad():
        ...
        #   acc = (output.argmax(dim=1) == test_label).sum().item()
            total_acc_test += acc
            part_acc_test += np.asarray(part_acc)
...
result_folder = "model_results_bart_transformer/"
...
# model = BartSeq2Seq(MAX_INPUT_LEN, MAX_TARGET_LEN, embed_size, action_len, ntokens, em_size, nhead, d_hid, nlayers, dropout).to(device)
model = Bart_Transformer_SE(MAX_INPUT_LEN, MAX_TARGET_LEN, embed_size, action_len, ntokens, em_size, nhead, d_hid, nlayers, dropout).to(device)
